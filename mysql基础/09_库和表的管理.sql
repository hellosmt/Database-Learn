...
CREATE TABLE IF NOT EXISTS stuinfo(
	stuid INT PRIMARY KEY AUTO_INCREMENT,#添加了主键约束
	stuname VARCHAR(20) UNIQUE NOT NULL,#添加了唯一约束+非空
	stugender CHAR(1) DEFAULT 'F',#添加了默认约束
	email VARCHAR(20) NOT NULL,
	age INT CHECK( age BETWEEN 0 AND 100),#添加了检查约束，mysql不支持
	majorid INT,
	CONSTRAINT fk_stuinfo_major FOREIGN KEY (majorid) REFERENCES major(m_id)#添加了外键约束
...
UNIQUE 唯一：用于限制该字段值不能重复
	...
	主键	×				1个
	唯一    √（空不算重复）				n个
CHECK检查：用于限制该字段值必须满足指定条件(mysql 不支持check)
...
FOREIGN KEY 外键:用于限制两个表的关系,要求外键列的值必须来自于主表的关联列
	...
	②主表的关联列要求必须是主键（mysql中unique也可以，但其它数据库大都只能是主键）
...
#案例：复制employees表中的last_name,department_id,salary字段到新表 emp表，但不复制数据（只复制部分结构）
